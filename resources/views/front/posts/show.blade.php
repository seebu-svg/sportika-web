        ...
        <section class="relative overflow-hidden">
            ...
            <div class="relative mx-auto max-w-4xl px-4 py-20 sm:px-6 lg:px-8">
                <nav class="mb-6 text-sm text-gray-400">
                    ...
                    <a href="{{ route('posts.index') }}" class="hover:text-accent-400">News</a>
                    <span class="mx-2 text-gray-700">/</span>
                    <span class="text-gray-700">{{ Str::limit($post->title, 40) }}</span>
                </nav>

                @if ($post->category)
                    <span class="inline-flex rounded-full bg-accent-500 px-4 py-1.5 text-xs font-bold uppercase tracking-[0.25em] text-black">
                        {{ $post->category->name }}
                    </span>
                @endif
        ...
        <section class="mx-auto max-w-4xl px-4 py-14 sm:px-6 lg:px-8">
            ...
            @if ($post->category)
                <div class="flex flex-wrap items-center gap-2">
                    <span class="text-xs font-bold uppercase tracking-wider text-gray-400">Tags:</span>
                    <a href="{{ route('posts.index', ['category' => $post->category->slug]) }}" class="rounded-full bg-blue-50 px-3 py-1 text-xs font-medium text-gray-700 transition hover:bg-accent-500/15 hover:text-accent-400">
                        {{ $post->category->name }}
                    </a>
                </div>
            @endif
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <button
                    ...
                    class="rounded-full border border-gray-200 px-4 py-2 text-xs font-medium text-gray-700 transition hover:border-accent-400 hover:text-accent-400"
                >
                    Copy Link
                </button>
                <a
                    ...
                    class="rounded-full border border-gray-200 px-4 py-2 text-xs font-medium text-gray-700 transition hover:border-green-500/50 hover:text-green-400"
                >
                    WhatsApp
                </a>
                <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    class="rounded-full border border-gray-200 px-4 py-2 text-xs font-medium text-gray-700 transition hover:border-sky-500/50 hover:text-sky-400"
                >
                    X (Twitter)
                </a>
            </div>
            ...
            <div class="flex flex-wrap items-center justify-between gap-4 border-t border-gray-200 pt-8">
                <a
                    ...
                    class="rounded-full border border-white/20 px-5 py-2.5 text-sm font-bold uppercase tracking-wider text-black transition hover:border-accent-400 hover:text-accent-400"
                >
                    &larr; All news
                </a>
                <a
                    href="{{ route('contact') }}"
                    class="rounded-full bg-accent-500 px-5 py-2.5 text-sm font-bold uppercase tracking-wider text-white transition hover:bg-accent-400"
                >
                    Contact us
                </a>
            </div>
