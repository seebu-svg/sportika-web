...
@section('content')
    <section class="relative overflow-hidden bg-pitch-900">
        ...
        <div class="relative mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
            ...
            <form method="GET" action="{{ route('tournaments.index') }}" class="grid gap-3 rounded-2xl border border-white/10 bg-pitch-800 p-4 sm:grid-cols-2 lg:grid-cols-4">
                <select name="sport" class="rounded-lg border border-white/10 bg-pitch-900 px-4 py-2.5 text-sm text-white outline-none focus:border-accent-400/60">
                ...
                <select name="status" class="rounded-lg border border-white/10 bg-pitch-900 px-4 py-2.5 text-sm text-white outline-none focus:border-accent-400/60">
                ...
                <div class="flex gap-2">
                    <button type="submit" class="flex-1 rounded-lg bg-accent-400 px-4 py-2.5 text-sm font-bold uppercase tracking-wider text-pitch-950 transition hover:bg-accent-300">Filter</button>
                    <a href="{{ route('tournaments.index') }}" class="rounded-lg border border-white/10 px-4 py-2.5 text-sm font-bold uppercase tracking-wider text-slate-300 transition hover:border-accent-400/50 hover:text-accent-300">Reset</a>
                </div>
            </form>
        </div>
    </section>
        ...
        @else
            <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                @foreach ($tournaments as $tournament)
                    <a href="{{ route('tournaments.show', $tournament->slug) }}" class="group relative overflow-hidden rounded-2xl border border-white/10 bg-pitch-800 p-6 transition duration-300 hover:-translate-y-1 hover:border-accent-400/40">
                        ...
                        <div class="relative">
                            @php
                                $statusColors = match($tournament->status) {
                                    ...
                                    default => 'bg-accent-400/15 text-accent-400',
                                };
                            @endphp
                            ...
                            @if ($tournament->sport)
                                <span class="ml-2 text-[11px] font-bold uppercase tracking-wider text-slate-500">{{ $tournament->sport }}</span>
                            @endif
