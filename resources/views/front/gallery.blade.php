        ...
        <div class="relative mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
            ...
            @if ($albums->isNotEmpty())
                <div class="flex flex-wrap gap-2">
                    <a href="{{ route('gallery') }}" class="rounded-full px-4 py-1.5 text-xs font-bold uppercase tracking-wider transition @unless(request('album')) bg-accent-400 text-pitch-950 @else border border-white/10 text-slate-300 hover:border-accent-400/50 hover:text-accent-300 @endunless">All</a>
                    @foreach ($albums as $album)
                        <a href="{{ route('gallery', ['album' => $album]) }}" class="rounded-full px-4 py-1.5 text-xs font-bold uppercase tracking-wider transition @if(request('album') === $album) bg-accent-400 text-pitch-950 @else border border-white/10 text-slate-300 hover:border-accent-400/50 hover:text-accent-300 @endif">
                            {{ $album }}
                        </a>
                    @endforeach
                </div>
        ...
        @else
            <div class="grid gap-4 grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                @foreach ($items as $item)
                    <div class="group relative overflow-hidden rounded-2xl border border-white/10 bg-pitch-800 @if($item->is_featured) md:col-span-2 md:row-span-2 @endif">
                        @if ($item->type === 'video' && str_contains($item->file_path, 'youtube'))
                            <div class="aspect-video flex items-center justify-center">
                                <div class="absolute inset-0 bg-diagonal"></div>
                                <div class="relative grid size-14 place-items-center rounded-full bg-accent-400/20 text-accent-400 transition group-hover:scale-110">
                        ...
                        @if ($item->title || $item->caption)
                            <div class="absolute inset-x-0 bottom-0 bg-gradient-to-t from-pitch-950/90 to-transparent p-4">
