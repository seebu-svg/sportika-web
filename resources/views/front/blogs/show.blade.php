    ...
    <article class="bg-pitch-900">
        ...
        <section class="relative overflow-hidden">
            ...
            <div class="relative mx-auto max-w-4xl px-4 py-20 sm:px-6 lg:px-8">
                <nav class="mb-6 text-sm text-slate-400">
                    ...
                    <a href="{{ route('blogs.index') }}" class="hover:text-accent-300">Blogs</a>
                    <span class="mx-2 text-slate-600">/</span>
                    <span class="text-slate-300">{{ Str::limit($post->title, 40) }}</span>
                </nav>
                @if ($post->category)
                    <span class="inline-flex rounded-full bg-accent-400 px-4 py-1.5 text-xs font-bold uppercase tracking-[0.25em] text-pitch-950">{{ $post->category->name }}</span>
                @endif
                ...
                @if ($post->excerpt)
                    <p class="mt-6 max-w-3xl text-lg leading-relaxed text-slate-300">{{ $post->excerpt }}</p>
                @endif
            </div>
        </section>
        ...
        <section class="mx-auto max-w-4xl px-4 py-14 sm:px-6 lg:px-8">
            ...
            <div class="mt-10 flex flex-wrap gap-2">
                ...
                <a href="https://wa.me/?text={{ urlencode($post->title.' '.route('blogs.show', $post->slug)) }}" target="_blank" class="rounded-full border border-white/10 px-3 py-1 text-xs text-slate-300 hover:border-green-500/50 hover:text-green-400">WhatsApp</a>
                <button onclick="navigator.clipboard.writeText('{{ route('blogs.show', $post->slug) }}'); this.textContent='Copied!';" class="rounded-full border border-white/10 px-3 py-1 text-xs text-slate-300 hover:border-accent-400/50 hover:text-accent-300">Copy Link</button>
            </div>

            <div class="mt-14 flex flex-wrap items-center justify-between gap-4 border-t border-white/10 pt-8">
                <a href="{{ route('blogs.index') }}" class="rounded-full border border-white/20 px-5 py-2.5 text-sm font-bold uppercase tracking-wider text-white transition hover:border-accent-400/60 hover:text-accent-300">&larr; All blogs</a>
                <a href="{{ route('contact') }}" class="rounded-full bg-accent-400 px-5 py-2.5 text-sm font-bold uppercase tracking-wider text-pitch-950 transition hover:bg-accent-300">Contact us</a>
            </div>
        </section>
    </article>
