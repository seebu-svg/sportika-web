...
@php
    ...
    $badgeColors = match($badge) {
        'verified' => 'bg-blue-600 text-pitch-950',
        'featured' => 'bg-amber-400 text-pitch-950',
        default => 'bg-slate-600 text-slate-600',
    };
@endphp
...
@section('content')
    ...
    <section class="relative overflow-hidden bg-blue-700">
        ...
        @else
            ...
            <div class="absolute -top-40 right-0 size-[32rem] rounded-full bg-blue-600/10 blur-3xl"></div>
        @endif
        ...
        <div class="relative mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
            <nav class="mb-6 text-sm text-slate-400">
                ...
                <a href="{{ route('players.index') }}" class="hover:text-blue-500">Players</a>
                <span class="mx-2 text-slate-600">/</span>
                <span class="text-slate-600">{{ $player->name }}</span>
            </nav>

            <div class="grid items-center gap-10 lg:grid-cols-5">
                <div class="lg:col-span-2">
                    <div class="relative mx-auto aspect-[3/4] max-w-sm overflow-hidden rounded-3xl border border-slate-200 bg-white">
                        <img
                            src="{{ $player->photo_url ?? asset('images/player-fallback.svg') }}"
                            alt="{{ $player->name }}"
                            class="size-full object-cover"
                        >
                        <div class="absolute inset-x-0 bottom-0 bg-gradient-to-t from-blue-900/70 to-transparent p-6">
                            @if ($player->jersey_number)
                                <span class="font-display text-7xl leading-none text-blue-600">
                ...
                <div class="lg:col-span-3">
                    <div class="flex flex-wrap items-center gap-3">
                        ...
                        @endif
                        @if ($player->level)
                            <span class="inline-flex rounded-full border border-slate-200 px-3 py-1.5 text-[11px] font-bold uppercase tracking-wider text-slate-400">
                                {{ $player->level }}
                            </span>
                        @endif
                    </div>

                    <h1 class="mt-4 font-display text-5xl uppercase tracking-wide text-pitch-950 sm:text-7xl">
                        {{ $player->name }}
                    </h1>

                    <div class="mt-5 flex flex-wrap gap-x-6 gap-y-2 text-sm text-slate-400">
                        @if ($player->current_club)
                            <span><strong class="text-pitch-950">Club:</strong> {{ $player->current_club }}</span>
                        @endif
                        @if ($player->city)
                            <span><strong class="text-pitch-950">City:</strong> {{ $player->city }}</span>
                        @elseif ($player->nationality)
                            <span><strong class="text-pitch-950">Nationality:</strong> {{ $player->nationality }}</span>
                        @endif
                        @if ($player->age)
                            <span><strong class="text-pitch-950">Age:</strong> {{ $player->age }}</span>
                        @endif
                    </div>
                    ...
                    <div class="mt-8 flex flex-wrap gap-3">
                        <a href="{{ route('contact') }}" class="rounded-full bg-blue-600 px-6 py-3 text-sm font-bold uppercase tracking-wider text-white transition hover:bg-blue-500">
                            Inquire about {{ Str::before($player->name, ' ') }}
                        </a>
                        <a href="{{ route('membership.player') }}" class="rounded-full border border-white/20 px-6 py-3 text-sm font-bold uppercase tracking-wider text-pitch-950 transition hover:border-blue-400 hover:text-blue-500">
                            Make Your Profile
                        </a>
                    </div>
                </div>
        ...
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            @foreach ($vitals as $vital)
                <div class="rounded-2xl border border-slate-200 bg-white p-5">
                    <p class="text-xs uppercase tracking-wider text-slate-400">{{ $vital['label'] }}</p>
                    <p class="mt-1 font-display text-2xl text-pitch-950">{{ $vital['value'] }}</p>
                </div>
            @endforeach
        </div>
            ...
            <ul class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                @if (! empty($player->achievements))
                    @foreach ($player->achievements as $achievement)
                        <li class="flex items-start gap-3 rounded-2xl border border-slate-200 bg-white p-5">
                            <span class="grid size-10 shrink-0 place-items-center rounded-full bg-blue-600/15 text-blue-600">
                            ...
                            <div>
                                <p class="font-semibold text-pitch-950">{{ $achievement['title'] ?? 'Achievement' }}</p>
                                @if (! empty($achievement['year']))
                                    <p class="text-xs text-blue-600">{{ $achievement['year'] }}</p>
                                @endif
                ...
                @if (! empty($player->honours))
                    @foreach ($player->honours as $award => $season)
                        <li class="flex items-start gap-3 rounded-2xl border border-slate-200 bg-white p-5">
                            ...
                            <div>
                                <p class="font-semibold text-pitch-950">{{ $award }}</p>
                        ...
                        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                            @foreach ($player->media as $item)
                                @if (str_contains($item['url'] ?? '', 'youtube') || str_contains($item['url'] ?? '', 'instagram'))
                                    <a href="{{ $item['url'] ?? '#' }}" target="_blank" rel="noopener" class="group relative aspect-video overflow-hidden rounded-2xl border border-slate-200 bg-white">
                                        <div class="bg-diagonal absolute inset-0"></div>
                                        <div class="absolute inset-0 flex flex-col items-center justify-center">
                                            <div class="grid size-12 place-items-center rounded-full bg-blue-600/20 text-blue-600 transition group-hover:scale-110">
                                ...
                                @else
                                    <div class="relative aspect-video overflow-hidden rounded-2xl border border-slate-200 bg-white">
                    ...
                    <div>
                        ...
                        <div class="space-y-3">
                            @foreach ($player->press_mentions as $mention)
                                <a href="{{ $mention['link'] ?? '#' }}" target="_blank" rel="noopener" class="flex items-center gap-4 rounded-2xl border border-slate-200 bg-white p-4 transition hover:border-blue-300">
                                    <span class="grid size-10 shrink-0 place-items-center rounded-full bg-blue-600/15 text-blue-600">
                                        <svg class="size-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 7.5h1.5m-1.5 3h1.5m-7.5 3h7.5m-7.5 3h7.5m3-9h3.375c.621 0 1.125.504 1.125 1.125V18a2.25 2.25 0 0 1-2.25 2.25M16.5 7.5V18a2.25 2.25 0 0 0 2.25 2.25M16.5 7.5V4.875c0-.621-.504-1.125-1.125-1.125H4.125C3.504 3.75 3 4.254 3 4.875V18a2.25 2.25 0 0 0 2.25 2.25h13.5M6 7.5h3v3H6v-3Z" /></svg>
                                    </span>
                                    <div>
                                        <p class="text-sm font-medium text-pitch-950">{{ $mention['publication'] ?? 'Press mention' }}</p>
                                        @if (! empty($mention['date']))
                                            <p class="text-xs text-slate-400">{{ $mention['date'] }}</p>
                                        @endif
                                    </div>
                                    <span class="ml-auto text-xs font-bold text-blue-600">Read →</span>
                                </a>
                            @endforeach
                        </div>
                    </div>
    ...
    <section class="mx-auto max-w-7xl px-4 pb-14 sm:px-6 lg:px-8">
        <div class="rounded-2xl border border-blue-400/20 bg-white p-6">
            <div class="flex flex-wrap items-center justify-between gap-6">
                <div>
                    <h3 class="font-display text-xl tracking-wider text-white">Share this profile</h3>
                    <div class="mt-3 flex flex-wrap gap-2">
                        <a href="https://wa.me/?text={{ urlencode($player->name.' — Player Profile '.route('players.show', $player->slug)) }}" target="_blank" rel="noopener" class="rounded-full border border-slate-200 px-4 py-2 text-sm text-slate-600 transition hover:border-green-500/50 hover:text-green-400">
                            WhatsApp
                        </a>
                        <a href="https://www.instagram.com/" target="_blank" rel="noopener" class="rounded-full border border-slate-200 px-4 py-2 text-sm text-slate-600 transition hover:border-pink-500/50 hover:text-pink-400">
                            Instagram Story
                        </a>
                        <button onclick="navigator.clipboard.writeText('{{ route('players.show', $player->slug) }}'); this.textContent='Copied!';" class="rounded-full border border-slate-200 px-4 py-2 text-sm text-slate-600 transition hover:border-blue-300 hover:text-blue-500">
                            Copy Link
                        </button>
                    </div>
                </div>
                <a href="{{ route('membership.player') }}" class="rounded-full bg-blue-600 px-6 py-3 text-sm font-bold uppercase tracking-wider text-white transition hover:bg-blue-500">
                    Make Your Profile
                </a>
            </div>
        </div>
