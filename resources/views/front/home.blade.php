...
@section('content')
    {{-- ============================== 1. HERO ============================== --}}
    <section class="relative flex min-h-[92vh] items-center overflow-hidden bg-blue-700">
        {{-- Animated background --}}
        <div class="absolute inset-0">
            <div class="bg-diagonal absolute inset-0"></div>
            <div class="absolute -top-40 -right-20 size-[40rem] rounded-full bg-blue-500/20 blur-3xl animate-pulse"></div>
            <div class="absolute -bottom-32 -left-20 size-[30rem] rounded-full bg-blue-400/10 blur-3xl"></div>
            {{-- Decorative grid lines --}}
            <div class="absolute inset-0 opacity-[0.04]" style="background-image: linear-gradient(to right, white 1px, transparent 1px), linear-gradient(to bottom, white 1px, transparent 1px); background-size: 80px 80px;"></div>
        </div>
        ...
        <div class="relative mx-auto max-w-7xl px-4 py-24 sm:px-6 lg:px-8">
            <div class="max-w-3xl">
                ...
                <div class="mt-10 flex flex-wrap gap-4">
                    <a href="{{ route('players.index') }}" class="rounded-full bg-white px-8 py-4 text-sm font-bold uppercase tracking-wider text-blue-700 transition hover:bg-blue-50">
                        Explore Players
                    </a>
    ...
    <section class="border-b border-slate-200 bg-pitch-50">
        <div class="mx-auto grid max-w-7xl items-center gap-14 px-4 py-20 sm:px-6 lg:grid-cols-2 lg:px-8">
            <div>
                ...
                <a
                    href="{{ route('about') }}"
                    class="mt-8 inline-block rounded-full bg-blue-600 px-6 py-3 text-sm font-bold uppercase tracking-wider text-white transition hover:bg-blue-700"
                >
                    Our Story &rarr;
                </a>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="rounded-2xl border border-slate-200 bg-white p-7 shadow-sm">
                    <p class="font-display text-5xl text-blue-600">{{ $stats['players'] }}</p>
                    <p class="mt-1 text-sm text-slate-500">Players represented</p>
                </div>
                <div class="rounded-2xl border border-slate-200 bg-white p-7 mt-6 shadow-sm">
                    <p class="font-display text-5xl text-blue-600">{{ $stats['countries'] }}</p>
                    <p class="mt-1 text-sm text-slate-500">Countries</p>
                </div>
                <div class="rounded-2xl border border-slate-200 bg-white p-7 shadow-sm">
                    <p class="font-display text-5xl text-blue-600">{{ $stats['tournaments'] }}</p>
                    <p class="mt-1 text-sm text-slate-500">Tournaments</p>
                </div>
                <div class="rounded-2xl border border-slate-200 bg-white p-7 mt-6 shadow-sm">
                    <p class="font-display text-5xl text-blue-600">{{ $stats['years'] }}+</p>
                    <p class="mt-1 text-sm text-slate-500">Years of experience</p>
                </div>
            </div>
        </div>
    </section>
    ...
    @if ($latestNews->isNotEmpty())
        <section class="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
            <div class="flex flex-wrap items-end justify-between gap-4">
                ...
                <a href="{{ route('posts.index') }}" class="mb-10 text-sm font-bold uppercase tracking-wider text-blue-600 transition hover:text-blue-500">
                    View All →
                </a>
            </div>
    ...
    @if ($latestBlogs->isNotEmpty())
        <section class="border-y border-slate-200 bg-pitch-50">
            <div class="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
                <div class="flex flex-wrap items-end justify-between gap-4">
                    <x-section-heading eyebrow="Insights & opinion" title="From the Blog" />
                    <a href="{{ route('blogs.index') }}" class="mb-10 text-sm font-bold uppercase tracking-wider text-blue-600 transition hover:text-blue-500">
                        View All →
                    </a>
                </div>
                <div class="grid gap-6 md:grid-cols-3">
    ...
    <section class="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <div class="flex flex-wrap items-end justify-between gap-4">
            ...
            />
            <a href="{{ route('tournaments.index') }}" class="mb-10 text-sm font-bold uppercase tracking-wider text-blue-600 transition hover:text-blue-500">
                View All →
            </a>
        </div>
        <div class="grid gap-6 md:grid-cols-3">
            @foreach ([
                ...
                <a href="{{ route('tournaments.index') }}" class="group relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-6 transition duration-300 hover:-translate-y-1 hover:border-blue-300">
                    <div class="bg-diagonal absolute inset-0 opacity-30"></div>
                    <div class="relative">
                        <span class="inline-block rounded-full bg-blue-600/15 px-3 py-1 text-[11px] font-bold uppercase tracking-wider text-blue-600">
                            {{ $tournament['status'] }}
                        </span>
                        <h3 class="mt-4 font-display text-xl uppercase tracking-wide text-pitch-950">{{ $tournament['name'] }}</h3>
    ...
    <section class="border-y border-slate-200 bg-blue-700">
        <div class="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
            <x-section-heading
                eyebrow="Watch"
            ...
            <div class="grid gap-6 md:grid-cols-3">
                @foreach (['Match Highlights — Matchday 12', 'Skills Compilation — Season Review', 'Behind the Scenes — Training Camp'] as $video)
                    <div class="group relative aspect-video overflow-hidden rounded-2xl border border-slate-200 bg-white">
                        <div class="bg-diagonal absolute inset-0"></div>
                        <div class="absolute inset-0 flex flex-col items-center justify-center">
                            <div class="grid size-16 place-items-center rounded-full bg-blue-600/20 text-blue-600 transition group-hover:scale-110 group-hover:bg-blue-600/30">
                                <svg class="size-7 ml-1" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>
                            </div>
                            <p class="mt-4 text-sm font-medium text-slate-600">{{ $video }}</p>
    ...
    @if ($featuredPlayers->isNotEmpty())
        <section class="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
            <div class="flex flex-wrap items-end justify-between gap-4">
                ...
                <a href="{{ route('players.index') }}" class="mb-10 text-sm font-bold uppercase tracking-wider text-blue-600 transition hover:text-blue-500">
                    View All →
                </a>
            </div>
    ...
    <section class="border-y border-slate-200 bg-pitch-50">
        <div class="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
            <div class="flex flex-wrap items-end justify-between gap-4">
                ...
                <a href="{{ route('gallery') }}" class="mb-10 text-sm font-bold uppercase tracking-wider text-blue-600 transition hover:text-blue-500">
                    View All →
                </a>
            </div>
            <div class="grid gap-4 grid-cols-2 md:grid-cols-4">
                @foreach (['Trophy Lift — Summer Cup 2025', 'Goal of the Season', 'Champions Celebration', 'MVP Award Ceremony'] as $i => $caption)
                    <div class="group relative aspect-square overflow-hidden rounded-2xl border border-slate-200 bg-white">
                        <div class="bg-diagonal absolute inset-0"></div>
                        <div class="absolute inset-0 flex flex-col items-center justify-center p-4 text-center">
                            <svg class="size-8 text-blue-600/40" fill="none" viewBox="0 0 24 24" stroke-width="1" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m2.25 15.45 5.73 3.12a11.998 11.998 0 0 1 0-15.14L2.25 6.55m15.5 8.9-5.73 3.12a11.998 11.998 0 0 0 0-15.14l5.73 3.12M6.75 12h.008v.008H6.75V12Z" /></svg>
                            <p class="mt-3 text-xs font-medium text-slate-400">{{ $caption }}</p>
                        </div>
                        <div class="absolute inset-0 bg-blue-600/0 transition group-hover:bg-blue-600/5"></div>
                    </div>
                @endforeach
            </div>
        </div>
    ...
    <section class="relative overflow-hidden">
        <div class="bg-diagonal absolute inset-0"></div>
        <div class="absolute -top-20 left-1/2 size-80 -translate-x-1/2 rounded-full bg-blue-600/5 blur-3xl"></div>
            ...
            <div class="grid grid-cols-2 gap-8 md:grid-cols-4">
                @foreach ([
                    ...
                    <div class="text-center">
                        <div class="mx-auto mb-4 grid size-14 place-items-center rounded-2xl border border-slate-200 bg-white">
                            <svg class="size-6 text-blue-600" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="{{ $stat['icon'] }}" />
                            </svg>
                        </div>
                        <p class="font-display text-5xl text-blue-600 sm:text-6xl">{{ $stat['value'] }}</p>
                        <p class="mt-2 text-sm uppercase tracking-wider text-slate-500">{{ $stat['label'] }}</p>
                    </div>
                @endforeach
            </div>
        </div>
    ...
    <section class="border-y border-slate-200 bg-blue-700">
        <div class="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
            ...
            <p class="mt-8 text-center">
                <a href="{{ route('sponsors') }}" class="text-sm font-bold uppercase tracking-wider text-white transition hover:text-blue-200">
    ...
    <section class="bg-white">
        <div class="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
            <div class="relative overflow-hidden rounded-3xl border border-blue-200 bg-gradient-to-br from-blue-700 to-blue-800 px-8 py-16 text-center sm:px-16">
                <div class="bg-diagonal absolute inset-0"></div>
                <div class="absolute -top-20 right-0 size-60 rounded-full bg-blue-500/20 blur-3xl"></div>
                <div class="relative">
                    ...
                    <div class="mt-10 flex flex-wrap justify-center gap-4">
                        <a
                            ...
                            class="rounded-full bg-white px-8 py-4 text-sm font-bold uppercase tracking-wider text-blue-700 transition hover:bg-blue-50"
                        >
                            Explore Players
                        </a>
                        <a
