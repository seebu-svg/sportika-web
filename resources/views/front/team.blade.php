    ...
    <section class="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        @if ($members->isEmpty())
            <div class="rounded-2xl border border-dashed border-white/15 bg-pitch-900 px-6 py-20 text-center">
                ...
                <a href="{{ route('about') }}" class="mt-6 inline-block rounded-full border border-accent-400/40 px-6 py-3 text-sm font-bold uppercase tracking-wider text-accent-300 transition hover:bg-accent-400/10">
                    Learn about us →
                </a>
            </div>
        @else
            <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                @foreach ($members as $member)
                    <a href="{{ route('team.show', $member->slug) }}" class="group relative overflow-hidden rounded-2xl border border-white/10 bg-pitch-800 transition duration-300 hover:-translate-y-1 hover:border-accent-400/40">
                        <div class="relative aspect-[3/4] overflow-hidden">
                            ...
                            >
                            <div class="absolute inset-0 bg-gradient-to-t from-pitch-950 via-pitch-950/40 to-transparent"></div>
                            <div class="absolute inset-x-0 bottom-0 p-5">
                                <h3 class="font-display text-2xl uppercase tracking-wide text-white">{{ $member->name }}</h3>
                                <p class="mt-1 text-sm font-medium text-accent-400">{{ $member->designation }}</p>
                            </div>
                        </div>
                        <div class="px-5 py-4">
                            @if ($member->bio)
